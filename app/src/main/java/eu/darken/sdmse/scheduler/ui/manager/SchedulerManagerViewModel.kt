    ...
    val items = combine(
        ...
        schedulerState.schedules.sortedBy { it.label.lowercase() }.map { schedule ->
            ScheduleRowVH.Item(
                ...
                onToggleVacuumApps = {
                    launch {
                        schedulerManager.saveSchedule(schedule.copy(useVacuumApps = !schedule.useVacuumApps))
                    }
                },
                onTogglePurgeLogs = {
                    launch {
                        schedulerManager.saveSchedule(schedule.copy(usePurgeLogs = !schedule.usePurgeLogs))
                    }
                },
                onEditFinalCommands = {
